...
**Atualização (Sprint 4.6, 06/09/2026):** magnitude exata quantificada — os 385 grupos envolvem **914 linhas (7,97% da população de 11.466 linhas)**, proporção estrutural, não marginal. Investigação de amostra representativa (órgão 244/MEC, 2020-06) confirmou `ingestion_timestamp` idêntico entre as linhas paralelas, descartando reprocessamento como causa; campos descritivos idênticos, apenas as medidas divergem — consistente com sub-populações legítimas publicadas pela fonte sem coluna discriminante, não com erro de ingestão. Tratamento `SUM` efetivamente implementado em `fct_depro_alocacao` (Sprint 4.6): grão físico usa `year`/`month` (não a string bruta `ano_mes_referencia`, por consistência com o padrão temporal do restante do projeto), resultado conferido exato contra gabarito pré-calculado (`11.466 − 914 + 385 = 10.937` linhas). Causa raiz permanece não determinável.