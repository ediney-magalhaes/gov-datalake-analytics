...
| 4.5 | Fato Afastamentos (grão de 5 colunas naturais, sem surrogate key — revalidado empiricamente) | ✅ Concluído (24/08/2026) |
| 4.6 | 3 Fatos DEPRO Alocação (grão `orgao + mês`, agregação `SUM`, achado de 7,97% de duplicidade de fonte), Cargos (grão de 6 colunas, exceção FUNAI/PECFUNAI), Aposentadorias Previstas (grão de 11 colunas, dois núcleos de classificação incompleta), nenhuma das 3 fontes possui `id_servidor_portal` (contradiz ADR-009, correção pendente) | ✅ Concluído (06/09/2026) |
...
**Achados da Sprint 4.5, relevantes para as próximas sprints:** primeiro fato da Fase 4 onde a revalidação empírica confirmou a hipótese da Sprint 3.5 sem alteração, grão de 5 colunas continua fechando 100% do resíduo, sentinela `-11` continua ausente. Critério de decisão para uso de surrogate key formalizado: chave natural enxuta (até ~5 colunas legíveis) não justifica `sk_*`; chaves mais extensas (7-8 colunas, casos de Vínculo/Ativos e Situação de Vínculo) sim. `fct_afastamentos` materializado, 9.069.323 linhas, 5/5 testes dbt aprovados.
**Achados da Sprint 4.6, relevantes para as próximas sprints:** primeira sprint da Fase 4 a sair do padrão "grão individual por servidor" (id_servidor_portal + tempo) que dominou 4.2-4.5 — os 3 fatos DEPRO são inerentemente pré-agregados por órgão, confirmado via schema real (nenhuma das 3 fontes possui `id_servidor_portal`, contradizendo o ADR-009, correção formal pendente). Grãos variam fortemente entre as 3 fontes de uma mesma origem (SEGES/Raio-X): Alocação fecha em 2 colunas com agregação SUM obrigatória (7,97% da população em duplicidade de fonte sem coluna discriminante, causa raiz não determinável); Cargos fecha em 6 colunas sem duplicidade nenhuma; Aposentadorias fecha em 11 colunas, também sem duplicidade, mas carrega duas dimensões temporais distintas (`year`/`month` de apuração vs. `ano_aposentadoria` de projeção) — nenhuma DEPRO herdou a composição de outra, mesmo princípio já visto entre Ativos e Aposentados na Sprint 4.4. Vocabulário de `grupo_cargo`/`cargo` testado entre Cargos e Aposentadorias mostrou 100% de divergência (0 sobreposição), decidindo por dimensões degeneradas em vez de uma `dim_cargo_depro` compartilhada — mesma lógica de incompatibilidade já registrada para órgão (SIAPE vs. DEPRO, Sprint 3.5). Dois núcleos de nulos não explicáveis por nenhuma coluna disponível na Prata foram isolados por padrão temporal (jul/2025-fev/2026, fev-mai/2024) e tratados via exclusão pontual no teste (`where`), preservando a linha em vez de descartar dado real. **Implicação para a Sprint 4.7 (Capacitação ENAP):** ao decidir as FKs e possíveis dimensões degeneradas de ENAP, testar explicitamente a sobreposição de vocabulário com outras fontes antes de presumir dimensão compartilhada — não repetir a suposição que só foi corrigida aqui por teste empírico direto. `fct_depro_alocacao` (10.937 linhas, 5/5 testes), `fct_depro_cargos` (142.404 linhas, 8/8 testes) e `fct_depro_aposentadorias` (257.547 linhas, 13/13 testes) materializados.
